...
setup() {
  ...
  TRASH_SCRIPT=$(command -v trash || command -v ts)
  if [ -z "$TRASH_SCRIPT" ]; then
      echo "trash/ts command not found; please install the trash tool."
      exit 1
  fi
...
@test "Check trash directory exists" {
  run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" -l
...
@test "Recover file from trash" {
  touch testfile.txt
  run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" testfile.txt
  run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" -r testfile.txt
  [ -f "testfile.txt" ]
}
...
@test "List with regex filter" {
  ...
  run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" file1.txt file2.txt
  run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" -l "file1"
  [ "$status" -eq 0 ]
  [[ "${output}" =~ "file1.txt" ]]
}
...
@test "Recursive listing" {
    ...
    run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" dir1
    run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" -l -R
    [ "$status" -eq 0 ]
    [[ "${output}" =~ "total" ]]
...
@test "Select specific folder" {
  ...
  touch dir1/dir2/file.txt
  run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" dir1
  run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" -l -s dir1
  [ "$status" -eq 0 ]
  [[ "${output}" =~ "dir1" ]]
}
...
@test "Empty trash older than N days" {
  touch oldfile.txt
  run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" oldfile.txt
  # Using --older 0 should remove all trashed items
  run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" -e --older 0
  [ ! -f "$XDG_DATA_HOME/Trash/files/oldfile.txt" ]
}

@test "Multiple file trash" {
  touch file1.txt file2.txt file3.txt
  run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" file1.txt file2.txt file3.txt
...
@test "Empty specific file from trash" {
  touch testfile.txt
  run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" testfile.txt
  run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" -e testfile.txt
  [ ! -f "$XDG_DATA_HOME/Trash/files/testfile.txt" ]
}
...
@test "Multiple file emptying" {
  touch file1.txt file2.txt file3.txt
  run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" file1.txt file2.txt file3.txt
  run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" -e file1.txt file2.txt file3.txt
  [ ! -f "$XDG_DATA_HOME/Trash/files/file1.txt" ]
  [ ! -f "$XDG_DATA_HOME/Trash/files/file2.txt" ]
  [ ! -f "$XDG_DATA_HOME/Trash/files/file3.txt" ]
}
...
@test "File collision handling" {
    touch original.txt
    run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" original.txt
    touch original.txt
    run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" original.txt
...
@test "Nested directory recovery with collisions" {
    ...
    mkdir -p dir1
    recovered_output=$(env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" -r dir1)
...
@test "Handle special characters in filenames" {
    ...
    run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" "file with spaces.txt" "file-with-@-symbol.txt" "file#with#hash.txt"
...
@test "Handle unicode characters in filenames" {
    ...
    run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" "file-with-üñîçødë.txt"
...
@test "Handle file with no read permission" {
    ...
    run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" "no_read_permission.txt"
...
@test "Handle file with no write permission" {
    ...
    run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" "no_write_permission.txt"
    [ -f "$XDG_DATA_HOME/Trash/files/no_write_permission.txt" ]
}

@test "Trash a broken symlink" {
    ln -s /nonexistent broken_link
    run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" broken_link
    [ "$status" -eq 0 ]
...
@test "Trashinfo stores symlink path not target" {
    ...
    run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" my_link
...
@test "Recover a broken symlink" {
    ...
    local orig_dir=$(pwd)
    run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" broken_link
    run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" -r broken_link
...
@test "Nonexistent file error" {
    run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" nonexistent_file
    [ "$status" -ne 0 ]
...
@test "Ambiguous recovery lists matches" {
    touch myfile.txt
    run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" myfile.txt
    touch myfile.txt
    run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" myfile.txt
    run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" -r myfile.txt
...
@test "Empty entire trash with --confirm" {
    touch file1.txt file2.txt
    run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" file1.txt file2.txt
    run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" -e --confirm
    [ "$status" -eq 0 ]
...
@test "Help menu displays" {
    run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" -h
    [ "$status" -eq 0 ]
    [[ "${output}" =~ "Trash Tool" ]]
    [[ "${output}" =~ "Usage:" ]]
}
...
@test "Recover with collision at original path" {
    ...
    run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" myfile.txt
    touch myfile.txt
    recovered_output=$(env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" -r myfile.txt)
...
@test "Trash and recover dotfile" {
    touch .hidden_file
    run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" .hidden_file
    [ "$status" -eq 0 ]
    run env XDG_DATA_HOME="$XDG_DATA_HOME" bash "$TRASH_TOOL_PATH/trash.sh" -r .hidden_file
    [ -f ".hidden_file" ]
}
